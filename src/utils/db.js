...
const dbDir = path.join(__basedir, 'data');
...
class StatementWrapper {
  ...
  _formatArgs(args) {
    ...
    // If first argument is an object (e.g. { points: -amount }) and followed by positional args
    if (args.length > 1 && args[0] !== null && typeof args[0] === 'object' && !Array.isArray(args[0])) {
      const flattened = [];
  ...
  get(...args) {
    try {
      ...
      const row = Array.isArray(formatted) ? this.stmt.get(...formatted) : this.stmt.get(formatted);
  ...
  all(...args) {
    try {
      const formatted = this._formatArgs(args);
      const rows = Array.isArray(formatted) ? this.stmt.all(...formatted) : this.stmt.all(formatted);
      if (!rows) return [];
      if (this._pluck) {
        return rows.map(r => {
  ...
  run(...args) {
    try {
      const formatted = this._formatArgs(args);
      return Array.isArray(formatted) ? this.stmt.run(...formatted) : this.stmt.run(formatted);
    } catch (err) {
...
class DBWrapper {
  ...
  pragma(sql) {
    try {
      ...
      // Ignored if pragma fails
    }
  }

  prepare(sql) {
    // Normalise @params in updatePoints query if needed
    let processedSql = sql;
    if (processedSql.includes('@points')) {
      processedSql = processedSql.replace(/@points/g, '?');
    }
